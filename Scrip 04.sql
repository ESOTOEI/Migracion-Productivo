...
-- Desarrollo: Identificación de sitios extendidos
-- Descripción: Adición de columna SITIO_EXTENDIDO en la tabla GES_SEDES_CLIENTES_PROCESO
-- ======================================================

BEGIN
    ------------------------------
    -- ADICIÓN DE COLUMNA SITIO_EXTENDIDO
    ------------------------------
    EXECUTE IMMEDIATE '
        ALTER TABLE "GES_SEDES_CLIENTES_PROCESO"
        ADD ("SITIO_EXTENDIDO" VARCHAR2(500))';

    DBMS_OUTPUT.PUT_LINE('Columna SITIO_EXTENDIDO agregada exitosamente a GES_SEDES_CLIENTES_PROCESO.');
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error al agregar la columna SITIO_EXTENDIDO: ' || SQLERRM);
END;
/
-- ======================================================
-- SCRIPT DE MODIFICACIONES
-- Desarrollo: Observaciones de atraso
-- Descripción: Creación de la tabla PRUEBA_OBSERVACIONES_ATRASO, 
-- su llave foránea y triggers asociados.
-- ======================================================

BEGIN
    ------------------------------
    -- CREACIÓN DE LA TABLA PRUEBA_OBSERVACIONES_ATRASO
    ------------------------------
    EXECUTE IMMEDIATE '
        CREATE TABLE "PRUEBA_OBSERVACIONES_ATRASO" (
            "ID" NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY 
                MINVALUE 1 MAXVALUE 9999999999999999999999999999 
                INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER NOCYCLE NOKEEP NOSCALE NOT NULL ENABLE,
            "OBSERVACION" CLOB NOT NULL ENABLE,
            "RESPONSABLE" VARCHAR2(255 CHAR) NOT NULL ENABLE,
            "NIVEL_ATRASO" VARCHAR2(255 CHAR),
            "PROCESO_ID_LOG" NUMBER,
            "CREATED" DATE,
            "CREATED_BY" VARCHAR2(255 CHAR),
            "UPDATED" DATE,
            "UDATED_BY" VARCHAR2(255 CHAR),
            "CREATED_TIME" VARCHAR2(8 CHAR),
            "UPDATED_TIME" VARCHAR2(8 CHAR),
            CONSTRAINT "PRUEBA_OBSERVACIONES_ATRASO_PK" PRIMARY KEY ("ID")
        )';

    ------------------------------
    -- LLAVE FORÁNEA A BPM_PROCESOS
    ------------------------------
    EXECUTE IMMEDIATE '
        ALTER TABLE "PRUEBA_OBSERVACIONES_ATRASO" 
        ADD CONSTRAINT "PRUEBA_OBSERVACIONES_ATRASO_BPM_FK"
        FOREIGN KEY ("PROCESO_ID_LOG") REFERENCES "BPM_PROCESOS" ("ID") ENABLE';

    ------------------------------
    -- TRIGGER DE AUDITORÍA BIU
    ------------------------------
    EXECUTE IMMEDIATE '
        CREATE OR REPLACE EDITIONABLE TRIGGER "TRG_PRUEBA_OBSERVACIONES_ATRASO_BIU"
        BEFORE INSERT OR UPDATE ON PRUEBA_OBSERVACIONES_ATRASO
        FOR EACH ROW
        BEGIN
            IF INSERTING THEN
                :NEW.CREATED := SYSDATE;
            ...
            END IF;
            :NEW.UPDATED := SYSDATE;
            :NEW.UDATED_BY := NVL(SYS_CONTEXT(''APEX$SESSION'',''APP_USER''), USER);
            :NEW.UPDATED_TIME := TO_CHAR(SYSDATE, ''HH24:MI:SS'');
        END;';

    EXECUTE IMMEDIATE 'ALTER TRIGGER "TRG_PRUEBA_OBSERVACIONES_ATRASO_BIU" ENABLE';

    ------------------------------
    -- TRIGGER DE ALERTA POST-INSERCIÓN
    ------------------------------
    EXECUTE IMMEDIATE '
        CREATE OR REPLACE EDITIONABLE TRIGGER "TRG_PRUEBA_OBSERVACIONES_ATRASO_ALERTA"
        AFTER INSERT ON PRUEBA_OBSERVACIONES_ATRASO
        FOR EACH ROW
        DECLARE
        ...
        BEGIN
            ...
            WHERE ID = (
                SELECT CLIENTE_ID 
                FROM BPM_PROCESOS 
                WHERE ID = :NEW.PROCESO_ID_LOG
            );

            enviar_alerta_observaciones(
                p_proceso_id => :NEW.PROCESO_ID_LOG,
                p_cliente => v_cliente,
                p_observaciones => :NEW.OBSERVACION,
                p_responsable => :NEW.RESPONSABLE
            );
        END;';

    EXECUTE IMMEDIATE 'ALTER TRIGGER "TRG_PRUEBA_OBSERVACIONES_ATRASO_ALERTA" ENABLE';

    DBMS_OUTPUT.PUT_LINE('Tabla PRUEBA_OBSERVACIONES_ATRASO, llave foránea y triggers creados exitosamente.');
EXCEPTION
    WHEN OTHERS THEN
...
-- ======================================================
-- SCRIPT DE MODIFICACIONES
-- Desarrollo: Gestión de responsables de atraso
-- Descripción: Creación de la tabla RESPONSABLES_ATRASO y su trigger de auditoría
-- ======================================================

BEGIN
    ------------------------------
    -- CREACIÓN DE LA TABLA RESPONSABLES_ATRASO
    ------------------------------
    EXECUTE IMMEDIATE '
        CREATE TABLE "RESPONSABLES_ATRASO" (
            "ID" NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY 
                MINVALUE 1 MAXVALUE 9999999999999999999999999999 
                INCREMENT BY 1 START WITH 1 CACHE 20 NOORDER NOCYCLE NOKEEP NOSCALE NOT NULL ENABLE,
            "CATEGORIA" VARCHAR2(255 CHAR) NOT NULL ENABLE,
            "CREATED" DATE DEFAULT SYSDATE,
            "CREATED_BY" VARCHAR2(255 CHAR),
            "UPDATED" DATE,
            "UPDATED_BY" VARCHAR2(255 CHAR),
            CONSTRAINT "RESPONSABLES_ATRASO_PK" PRIMARY KEY ("ID")
        )';
    ...
    ------------------------------
    EXECUTE IMMEDIATE '
        CREATE OR REPLACE EDITIONABLE TRIGGER "RESPONSABLES_ATRASO_BIU"
        BEFORE INSERT OR UPDATE ON RESPONSABLES_ATRASO
        FOR EACH ROW
        BEGIN
            IF INSERTING THEN
                :NEW.CREATED := SYSTIMESTAMP AT TIME ZONE ''America/Bogota'';
    ...
    EXECUTE IMMEDIATE 'ALTER TRIGGER "RESPONSABLES_ATRASO_BIU" ENABLE';

    DBMS_OUTPUT.PUT_LINE('Tabla RESPONSABLES_ATRASO y trigger creados exitosamente.');
EXCEPTION
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Error en la creación: ' || SQLERRM);
END;
